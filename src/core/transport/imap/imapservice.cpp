...
void
ImapService::prefetchAttachment(const QString &accountEmail, const QString &folderName, const QString &uid, const QString &partId) {
    if (!m_store || !m_accounts || partId.trimmed().isEmpty())
        return;
    ...
    for (const auto &a : attachments) {
        const auto am = a.toMap();
        if (am.value("partId"_L1).toString() == partId) {
            target = am;
            break;
        }
    }
    ...
    runBackgroundTask([this, host, port, accountEmail, folderName, uid, account, target]() {
        const QString token = workerRefreshAccessToken(account, accountEmail);
        if (token.isEmpty())
            return;
        ...
        const QString partId   = target.value("partId"_L1).toString();
        const QString encoding = target.value("encoding"_L1).toString();
        const QString name     = target.value("name"_L1).toString();
        const QString key      = attachmentCacheKey(accountEmail, uid, partId);
